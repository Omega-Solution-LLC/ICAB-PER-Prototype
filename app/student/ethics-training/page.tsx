...
import { ETHICS_MODULES_SEED } from "@/lib/constants";
import { EthicsModule } from "@/types";
import { CheckCircle2, Eye, ShieldCheck, ChevronLeft, ChevronRight } from "lucide-react";
...
function getModuleStatus(modules: EthicsModule[]) {
  if (modules.length === 0) return "not-started" as const;
  if (modules.every((r) => r.status === "approved")) return "approved" as const;
  if (modules.some((r) => r.status === "submitted")) return "submitted" as const;
...
export default function EthicsTrainingPage() {
  ...
  const [approvedPreview, setApprovedPreview] = useState<EthicsModule | null>(null);
  const [selectedModuleName, setSelectedModuleName] = useState<string>(ETHICS_MODULES_SEED[0]?.name || "");
  ...
  const selectedModuleRecords = ethicsModules.filter((module) => module.name === selectedModuleName);
  ...
  const handleOpenResponse = (module: EthicsModule) => {
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="border border-gray-200 mb-6">
          <div className="bg-gray-700 text-white text-left px-5 py-3 text-sm font-semibold tracking-wide flex items-center gap-2">
            ...
            Ethics Assignment Workflow
          </div>
          <div className="bg-gray-50 px-5 py-5">
            <p className="text-sm text-gray-600 leading-relaxed font-normal">
              For each ethics module, submit a written response to the assignment question.
        ...
        <div className="mb-6 border border-gray-200">
          ...
          <div className="bg-gray-50 border-t-0 border-gray-200 px-5 py-6">
            <div className="relative">
              ...
              >
                {ETHICS_MODULES_SEED.map((module) => {
                  const moduleStatus = getModuleStatus(ethicsModules.filter((item) => item.name === module.name));
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-xs text-gray-600 mt-2">{module.description}</p>
                    </button>
                  );
                })}
              </div>
                    ...
                    <tr key={module.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-4 text-base text-left text-gray-900 leading-snug">
                        {isApproved ? (
                          ...
                          <p className="font-semibold text-gray-900 mb-1">{module.responseQuestion}</p>
                        )}
